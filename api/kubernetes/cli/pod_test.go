...
func TestSupportsPodRestart(t *testing.T) {
	...
	t.Run("returns false when pods/restart is absent from discovery", func(t *testing.T) {
		t.Parallel()
		...
		ok, err := kcl.SupportsPodRestart()
		require.NoError(t, err)
		assert.False(t, ok)
	})

	t.Run("returns true when pods/restart is present in discovery", func(t *testing.T) {
		t.Parallel()
		fakeClient := kfake.NewSimpleClientset()
		fakeClient.Resources = []*metav1.APIResourceList{
			{
	...
	t.Run("returns false when v1 resource list is empty", func(t *testing.T) {
		t.Parallel()
		fakeClient := kfake.NewSimpleClientset()
		fakeClient.Resources = []*metav1.APIResourceList{
	...
	t.Run("propagates discovery error when v1 group is absent", func(t *testing.T) {
		t.Parallel()
		fakeClient := kfake.NewSimpleClientset()
		// Only register a non-v1 group so that ServerResourcesForGroupVersion("v1") fails.
		fakeClient.Resources = []*metav1.APIResourceList{
